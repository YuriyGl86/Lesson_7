class Game {
  ...
  eventHandler(event){
      console.log(event.key.charCodeAt(0), this.currentSymbol.textContent.charCodeAt(0));
      console.log(event.key);

      if(event.key == 'Shift' || event.key == 'Alt' || event.key == 'Control') return
      if(event.key.charCodeAt(0) == this.currentSymbol.textContent.charCodeAt(0)) this.success();
      else this.fail();
  }
